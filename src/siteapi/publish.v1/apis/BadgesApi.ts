...
export class BadgesApiRequestFactory extends BaseAPIRequestFactory {
     ...
     * @param files 
     */
    public async v1BadgesBadgeIdIconPost(badgeId: number, files?: HttpFile, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // verify required parameter 'badgeId' is not null or undefined
        if (badgeId === null || badgeId === undefined) {
            throw new RequiredError("BadgesApi", "v1BadgesBadgeIdIconPost", "badgeId");
        }



        // Path Params
        const localVarPath = '/v1/badges/{badgeId}/icon'
...
export class BadgesApiResponseProcessor {
     ...
     *
     * @params response Response returned by the server for a request to v1BadgesBadgeIdIconPost
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async v1BadgesBadgeIdIconPostWithHttpInfo(response: ResponseContext): Promise<HttpInfo<RobloxPublishApiUploadResponse >> {
        ...
        if (isCodeInRange("400", response.httpStatusCode)) {
            throw new ApiException<undefined>(response.httpStatusCode, "2: File not present in request.  12: Name or description is moderated.", undefined, response.headers);
        }
