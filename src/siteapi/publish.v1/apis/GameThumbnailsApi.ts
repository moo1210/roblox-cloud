 ...
 * no description
 */
export class GameThumbnailsApiRequestFactory extends BaseAPIRequestFactory {
     ...
     * @param files 
     */
    public async v1GamesGameIdThumbnailImagePost(gameId: number, files?: HttpFile, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // verify required parameter 'gameId' is not null or undefined
        if (gameId === null || gameId === undefined) {
            throw new RequiredError("GameThumbnailsApi", "v1GamesGameIdThumbnailImagePost", "gameId");
        }



        // Path Params
        const localVarPath = '/v1/games/{gameId}/thumbnail/image'
...
export class GameThumbnailsApiResponseProcessor {

    /**
     * Unwraps the actual response sent by the server from the response context and deserializes the response content
     * to the expected objects
     *
     * @params response Response returned by the server for a request to v1GamesGameIdThumbnailImagePost
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async v1GamesGameIdThumbnailImagePostWithHttpInfo(response: ResponseContext): Promise<HttpInfo<RobloxPublishApiUploadResponse >> {
        ...
        if (isCodeInRange("400", response.httpStatusCode)) {
            throw new ApiException<undefined>(response.httpStatusCode, "1: File uploaded does not match known image format. Try converting to png.  2: File not present in request.", undefined, response.headers);
        }
