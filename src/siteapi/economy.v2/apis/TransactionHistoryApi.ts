...
export class TransactionHistoryApiRequestFactory extends BaseAPIRequestFactory {
     ...
     */
    public async v2GroupsGroupIdTransactionsGet(groupId: number, transactionType: 'Sale' | 'Purchase' | 'AffiliateSale' | 'DevEx' | 'GroupPayout' | 'AdImpressionPayout' | 'CurrencyPurchase' | 'TradeRobux' | 'PremiumStipend' | 'PendingRobux' | 'EngagementPayout' | 'GroupEngagementPayout' | 'AdSpend' | 'Summary' | 'IndividualToGroup' | 'CSAdjustment', limit?: 10 | 25 | 50 | 100, cursor?: string, sortOrder?: 'Asc' | 'Desc', _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // verify required parameter 'groupId' is not null or undefined
        if (groupId === null || groupId === undefined) {
            throw new RequiredError("TransactionHistoryApi", "v2GroupsGroupIdTransactionsGet", "groupId");
        }


        // verify required parameter 'transactionType' is not null or undefined
        if (transactionType === null || transactionType === undefined) {
            throw new RequiredError("TransactionHistoryApi", "v2GroupsGroupIdTransactionsGet", "transactionType");
        }
        ...
        // Path Params
        const localVarPath = '/v2/groups/{groupId}/transactions'
...
export class TransactionHistoryApiResponseProcessor {
     ...
     *
     * @params response Response returned by the server for a request to v2GroupsGroupIdTransactionsGet
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async v2GroupsGroupIdTransactionsGetWithHttpInfo(response: ResponseContext): Promise<HttpInfo<RobloxWebWebAPIModelsApiPageResponseRobloxEconomyApiModelsTransactionResponse >> {
